...
function ThreadPane({
  ...
  useEffect(() => {
    if (!focused || !stickToBottom) return;
    bottomRef.current?.scrollIntoView({ behavior: 'smooth' });
  }, [thread.messages, isStreamingHere, focused, stickToBottom]);

  function handleMessageListScroll(): void {
    const el = scrollRef.current;
    if (!el) return;
    setStickToBottom(el.scrollHeight - el.scrollTop - el.clientHeight <= 96);
  }
  ...
  return (
    ...
    >
      ...
      <div
        key={`msglist-${thread.id}`}
        ref={scrollRef}
        onScroll={handleMessageListScroll}
        className="h-full overflow-y-auto px-4 py-4 animate-content-switch"
      >
