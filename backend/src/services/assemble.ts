...
function roomPersonalitySystem(
  locale: Locale,
  contact: { name: string; personality_prompt: string },
): ChatRoleMessage {
  return {
    role: 'system',
    content: t(locale, 'assemble.roomPersonality', {
      name: contact.name,
      prompt: contact.personality_prompt.trim() || contact.name,
    }),
  };
}

/**
 * Idle room context bar: history + personality of the likely next speaker
 * (passed contact, else empty personality omitted).
 */
export function buildRoomContextSnapshot(
  threadId: string,
  settings: AppSettings,
  contact?: { id: string; name: string; personality_prompt: string } | null,
): AssembledContext {
  const ctxSettings = contextSettingsFromApp(settings);
  const locale = localeOf(settings);
  const history = toRoleMessages(getThreadMessages(threadId));
  const assembled: ChatRoleMessage[] = contact
    ? [roomPersonalitySystem(locale, contact), ...history]
    : history;
  const protectPrefixCount = contact ? 1 : 0;
  const fullTokens = estimateMessagesTokens(assembled);
  const snapshot = buildContextSnapshot(
    assembled.map((m, i) => ({
      id: m.id ?? `room-${i}`,
  ...
  return {
    ...
    window: settings.contextWindow,
    snapshot,
    upstreamCount: protectPrefixCount,
  };
}

export function prepareRoomChatMessages(options: {
  threadId: string;
  settings: AppSettings;
  contact: { id: string; name: string; personality_prompt: string };
  pendingUserContent: string;
  quoteExcerpt?: string | null;
}): {
  ok: true;
  selected: Array<{ role: 'user' | 'assistant' | 'system'; content: string }>;
  snapshot: ContextSnapshot;
  targetContactId: string;
} {
  const locale = localeOf(options.settings);
  const ctxSettings = contextSettingsFromApp(options.settings);
  const history = toRoleMessages(getThreadMessages(options.threadId));

  let userContent = options.pendingUserContent;
  const quote = options.quoteExcerpt?.trim();
  if (quote) {
    userContent = t(locale, 'assemble.roomQuote', {
      quote,
      content: options.pendingUserContent,
    });
  }

  const assembled: ChatRoleMessage[] = [
    roomPersonalitySystem(locale, options.contact),
    ...history,
    { role: 'user', content: userContent },
  ];

  const { selected } = buildSlidingWindow(assembled, ctxSettings, {
    protectPrefixCount: 1,
  });
  ...
  return {
    ok: true,
    selected: selected.map(({ role, content }) => ({ role, content })),
    snapshot,
    targetContactId: options.contact.id,
  };
}

/** Ephemeral prompt for meeting auto-speak — no user message persisted. */
export function prepareMeetingSpeakMessages(options: {
  threadId: string;
  settings: AppSettings;
  contact: { id: string; name: string; personality_prompt: string };
  meetingGoal: string;
  ...
  const locale = localeOf(options.settings);
  const ctxSettings = contextSettingsFromApp(options.settings);
  const history = toRoleMessages(
    getThreadMessages(options.threadId, { sinceCreatedAt: options.sinceCreatedAt }),
  );
  const userContent = t(locale, 'assemble.meetingSpeak', { goal: options.meetingGoal });
