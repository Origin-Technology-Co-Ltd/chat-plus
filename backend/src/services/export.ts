...
function renderMessageBlock(message: MessageRow): string {
  ...
  if (message.role === 'assistant') {
    if (message.contact_id) {
      ...
      if (contact) {
        lines.push(`contact: ${JSON.stringify(contact.name)}`, '');
      }
    }
    if (message.model_label?.trim()) {
      lines.push(`model: ${JSON.stringify(message.model_label.trim())}`, '');
    }
  }
  if (message.role === 'user' && message.target_contact_id) {
    const contact = getContact(message.target_contact_id);
    if (contact) {
      lines.push(`to: ${JSON.stringify(contact.name)}`, '');
    }
  }
